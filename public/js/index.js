...
function randerDate() {
    ...
    let currentDate = new Date(newDateFunction.getFullYear(), newDateFunction.getMonth() + 1, 0).getDate(); // to get the last date of current month

    let prevDate = new Date(newDateFunction.getFullYear(), newDateFunction.getMonth(), 0).getDate(); // to get the last date of previous month

    let addNext = new Date(newDateFunction.getFullYear(), newDateFunction.getMonth() + 2, 0).getDate() + 7;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.onclick = function (event) {
        if (
            event.target.classList.contains("popup-booking") ||
            event.target.classList.contains("popup-events") ||
            event.target.classList.contains("popup-payment")
        ) {
    ...
    const financeChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    // Adding title configuration here
                    display: true,
...
/*using jequery to toggle different content-finance payment overview*/

$(document).ready(function () {
    // Initially hide all content
    $(".payment-details").hide();

    // Handling clicks on the invoice link (this should navigate away to the invoice page)
    $('a[href="payment_overview.php"]').click(function (e) {
        $(".payment-details").hide();
        $("#all-payments-content").show();
        // Not preventing default behavior, so it will navigate to payment_invoice.php
    });

    // Upcoming payments link
    $('a[href="payment_up_coming.php"]').click(function (e) {
        e.preventDefault(); // Prevent the default anchor behavior
        $(".payment-details").hide();
        $("#upcoming-payments-content").show();
    });

    // Due payments link
    $('a[href="payment_due.php"]').click(function (e) {
        e.preventDefault();
        $(".payment-details").hide();
        $("#due-payments-content").show();
    });

    // History payments link
    $('a[href="payment_log.php"]').click(function (e) {
        e.preventDefault();
        $(".payment-details").hide();
        $("#history-payments-content").show();
    });
});
